...
namespace Controls.Core.Form
{
    ...
    public class FormComposer
    {
        ...
        /// <returns></returns>
        public static HtmlString composeForm(string formId, string formObject = null, string title = null, string formTitle = null, string content = null, string actions = null, string createFunction = null, string updateFunction = null, string scopeObject = null, bool modal = true, bool display = false)
        {   
            ...
            if (modal)
            {
                ...
                modalFormCrossButton = "<button type = 'button' class='col order-1 close d-flex justify-content-end' data-dismiss='modal'> <span aria-hidden='true'>&times;</span><span class='sr-only'>" + SharedLibrary.Core.Resources.Global.FormMessages.Close + "</span></button> ";
                formCloseButton = "<button type = 'button' id ='btnCloseAddForm' class='btn btn-default' data-dismiss='modal'><span>Close</span></button>";
                formOkButton = "<button type = 'button' id='btnOK' class='btn btn-primary' data-dismiss='modal'> <span>" + SharedLibrary.Core.Resources.Global.FormMessages.Done + "</span></button>";
                modalDialogAttributes = "class='modal-dialog modal-dialog-centered'";
                modalContentAttributes = "class='modal-content'";
            }
            else
            {
                formCloseButton = "<button type = \"button\" onclick=\"javascript: $('#" + formId + "').dialog('close');\" id=\"btnCloseAddForm\" class=\"btn btn-default\"><span>" + SharedLibrary.Core.Resources.Global.FormMessages.Close + "</span></button>";
                formOkButton = "<button type = \"button\" onclick=\"javascript: $('#" + formId + "').dialog('close');\" id=\"btnOK\" class=\"btn btn-default\"><span>" + SharedLibrary.Core.Resources.Global.FormMessages.Done + "</span></button>";
            }
