...
namespace ApplicationForms
{
    ...
    public class ChannelsAndBindings<T> where T : class
    {
        ...
        {
            EndpointAddress endPoint = new EndpointAddress(address);
            DuplexChannelFactory<IServiceCB<T>> factory = new DuplexChannelFactory<IServiceCB<T>>(callbackObject, (NetTcpBinding)GetDuplexTCPBinding(secured), endPoint);
        ...
        {
            EndpointAddress endPoint = new EndpointAddress(address);
            DuplexChannelFactory<IServiceCBAsync<T>> factory = new DuplexChannelFactory<IServiceCBAsync<T>>(callbackObject, (NetTcpBinding)GetDuplexTCPBinding(secured), endPoint);
        ...
        {
            EndpointAddress endPoint = new EndpointAddress(address);
            DuplexChannelFactory<IWCFServiceCB<T>> factory = new DuplexChannelFactory<IWCFServiceCB<T>>(callbackObject, (NetTcpBinding)GetDuplexTCPBinding(secured), endPoint);
        ...
        {
            EndpointAddress endPoint = new EndpointAddress(address);
            DuplexChannelFactory<IWCFServiceCBAsync<T>> factory = new DuplexChannelFactory<IWCFServiceCBAsync<T>>(callbackObject, (NetTcpBinding)GetDuplexTCPBinding(secured), endPoint);
        ...
        #region GetDuplexBasicHttpChannelFactory
        /// <summary>
        /// Gets synchronous channel factory.
        /// </summary>
        /// <returns></returns>
        public static DuplexChannelFactory<IServiceCB<T>> GetDuplexBasicHttpChannelFactory(string address, object callbackObject, bool secured = false)
        {
            EndpointAddress endPoint = new EndpointAddress(address);
            DuplexChannelFactory<IServiceCB<T>> factory = null;

            if (secured)
            {
                factory = new DuplexChannelFactory<IServiceCB<T>>(callbackObject, (WSDualHttpBinding)GetDuplexHttpBinding(secured), endPoint);
                factory.Credentials.ServiceCertificate.SslCertificateAuthentication = new X509ServiceCertificateAuthentication();
                factory.Credentials.ServiceCertificate.SslCertificateAuthentication.CertificateValidationMode = X509CertificateValidationMode.None;
            }
            else
        ...
        #region GetDuplexBasicHttpChannelAsyncFactory
        /// <summary>
        /// Gets asynchronous channel factory.
        /// </summary>
        /// <returns></returns>
        public static DuplexChannelFactory<IServiceCBAsync<T>> GetDuplexBasicHttpChannelAsyncFactory(string address, object callbackObject, bool secured = false)
        {
            EndpointAddress endPoint = new EndpointAddress(address);
            DuplexChannelFactory<IServiceCBAsync<T>> factory = null;

            if (secured)
            {
                factory = new DuplexChannelFactory<IServiceCBAsync<T>>(callbackObject, (WSDualHttpBinding)GetDuplexHttpBinding(secured), endPoint);
                factory.Credentials.ServiceCertificate.SslCertificateAuthentication = new X509ServiceCertificateAuthentication();
                factory.Credentials.ServiceCertificate.SslCertificateAuthentication.CertificateValidationMode = X509CertificateValidationMode.None;
            }
            else
        ...
        #region GetDuplexBasicHttpWCFChannelFactory
        /// <summary>
        /// Gets synchronous channel factory.
        /// </summary>
        /// <returns></returns>
        public static DuplexChannelFactory<IWCFServiceCB<T>> GetDuplexBasicHttpWCFChannelFactory(string address, object callbackObject, bool secured = false)
        {
            EndpointAddress endPoint = new EndpointAddress(address);
            DuplexChannelFactory<IWCFServiceCB<T>> factory = null;

            if (secured)
            {
                factory = new DuplexChannelFactory<IWCFServiceCB<T>>(callbackObject, (WSDualHttpBinding)GetDuplexHttpBinding(secured), endPoint);
                factory.Credentials.ServiceCertificate.SslCertificateAuthentication = new X509ServiceCertificateAuthentication();
                factory.Credentials.ServiceCertificate.SslCertificateAuthentication.CertificateValidationMode = X509CertificateValidationMode.None;
            }
            else
        ...
        #region GetDuplexBasicHttpWCFChannelAsyncFactory
        /// <summary>
        /// Gets asynchronous channel factory.
        /// </summary>
        /// <returns></returns>
        public static DuplexChannelFactory<IWCFServiceCBAsync<T>> GetDuplexBasicHttpWCFChannelAsyncFactory(string address, object callbackObject, bool secured = false)
        {
            EndpointAddress endPoint = new EndpointAddress(address);
            DuplexChannelFactory<IWCFServiceCBAsync<T>> factory = null;

            if (secured)
            {
                factory = new DuplexChannelFactory<IWCFServiceCBAsync<T>>(callbackObject, (WSDualHttpBinding)GetDuplexHttpBinding(secured), endPoint);
                factory.Credentials.ServiceCertificate.SslCertificateAuthentication = new X509ServiceCertificateAuthentication();
                factory.Credentials.ServiceCertificate.SslCertificateAuthentication.CertificateValidationMode = X509CertificateValidationMode.None;
            }
            else
